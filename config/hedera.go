...
func HederaPublicKey() hedera.PublicKey {
	operatorPublicKey, err := hedera.PublicKeyFromString(Config("HEDERA_ACCOUNT_PRIVATE_KEY"))
	if err != nil {
		panic(err)
	}
...
func HederaPrivateKey() hedera.PrivateKey {
	operatorPrivateKey, err := hedera.PrivateKeyFromString(Config("HEDERA_ACCOUNT_PUBLIC_KEY"))
	if err != nil {
		panic(err)
	}
